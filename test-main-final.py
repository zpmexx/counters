...
def subscribe(client: mqtt_client):
    ...
    def on_message(client, userdata, msg):
        ...
        date_obj = datetime.strptime(recivedDate, "%Y-%m-%d")
        recived_date_only = date_obj.date()
        today = datetime.today()
        today_date_only = today.date()
        older = today_date_only + timedelta(30)
        younger = today_date_only - timedelta(30)
        print('poczatkowa data otrzymana')
        print(recivedDate)
        print("dzisiejsza data")
        print(today_date_only)
        print('otrzymana data')
        print(recived_date_only)
        if younger < recived_date_only < older:
            print("poprawna")
        else:
            print("niepoprawna")
